...
! noahmp-style TBLs read routines:
...
! Public subroutines/functions
    ...
    public :: read_mpt_veg_parameters
    public :: read_mpt_soil_parameters
    public :: read_mpt_rad_parameters

CONTAINS

  SUBROUTINE read_mpt_veg_parameters(param_dir, mpt_table, DATASET_IDENTIFIER)
    implicit none
    character(len=*), intent(in) :: param_dir
    character(len=*), intent(in) :: mpt_table
    ...
    namelist / mpt_usgs_veg_categories / VEG_DATASET_DESCRIPTION, NVEG

    namelist / mpt_usgs_parameters / ISURBAN, ISWATER, ISBARREN, ISICE, ISCROP, EBLFOREST, NATURAL, &
    ...
    namelist / mpt_modis_veg_categories / VEG_DATASET_DESCRIPTION, NVEG

    namelist / mpt_modis_parameters / ISURBAN, ISWATER, ISBARREN, ISICE, ISCROP, EBLFOREST, NATURAL, &
         LCZ_1,LCZ_2,LCZ_3,LCZ_4,LCZ_5,LCZ_6,LCZ_7,LCZ_8,LCZ_9,LCZ_10,LCZ_11, &
         CH2OP, DLEAF, Z0MVT, HVT, HVB, DEN, RC, MFSNO, SCFFAC, XL, CWPVT, C3PSN, KC25, AKC, KO25, AKO, AVCMX, AQE, &
         LTOVRC,  DILEFC,  DILEFW,  RMF25 ,  SLA   ,  FRAGR ,  TMIN  ,  VCMX25,  TDLEF ,  BP, MP, QE25, RMS25, RMR25, ARM, &
         FOLNMX, WDPOOL, WRRAT, MRP, SHDFAC, NROOT, RGL, RS, HS, TOPT, RSMAX, &
    ...
    inquire( file=trim(param_dir)//'/'//trim(mpt_table), exist=file_named )
    if ( file_named ) then
      open(15, file=trim(param_dir)//'/'//trim(mpt_table), status='old', form='formatted', action='read', iostat=ierr)
    else
      open(15, status='old', form='formatted', action='read', iostat=ierr)
    end if

    if (ierr /= 0) then
       call handle_err(ierr, "ParametersRead.f90: read_mpt_veg_parameters: Cannot find file MPTABLE.TBL")
    endif

    if ( trim(DATASET_IDENTIFIER) == "USGS" ) then
       read(15,mpt_usgs_veg_categories)
       read(15,mpt_usgs_parameters)
    else if ( trim(DATASET_IDENTIFIER) == "MODIFIED_IGBP_MODIS_NOAH" ) then
       read(15,mpt_modis_veg_categories)
       read(15,mpt_modis_parameters)
    else
       write(*,'("WARNING: DATASET_IDENTIFIER = ''", A, "''")') trim(DATASET_IDENTIFIER)
       call handle_err(ierr, 'ParametersRead.f90: read_mpt_veg_parameters: Unrecognized DATASET_IDENTIFIER in subroutine READ_MP_VEG_PARAMETERS')
    endif
    close(15)
  ...
  END SUBROUTINE read_mpt_veg_parameters

  SUBROUTINE read_mpt_soil_parameters(param_dir, soil_table, general_table, soil_class_name)
    implicit none
    character(len=*), intent(in) :: param_dir
...
!
    ...
    if (ierr/=0) then
      write(message,fmt='(A)') 'ParametersRead.f90: read_mpt_soil_parameters: failure opening SOILPARM.TBL'
...
!
    ...
    if (ierr /= 0) then
      call handle_err(ierr, 'ParametersRead.f90: read_mpt_soil_parameters: failure opening GENPARM.TBL')
    end if
  ...
  END SUBROUTINE read_mpt_soil_parameters


  SUBROUTINE read_mpt_rad_parameters(param_dir, mpt_table)
    implicit none
    character(len=*), intent(in) :: param_dir
    character(len=*), intent(in) :: mpt_table
    integer                      :: ierr
    ...
    namelist / mpt_rad_parameters / ALBSAT_VIS,ALBSAT_NIR,ALBDRY_VIS,ALBDRY_NIR,ALBICE,ALBLAK,OMEGAS,BETADS,BETAIS,EG
    ...
    if (ierr /= 0) then
       call handle_err(ierr, 'ParametersRead.f90: read_mpt_rad_parameters: Cannot find file MPTABLE.TBL')
    endif

    read(15,mpt_rad_parameters)
    close(15)
  ...
  end subroutine read_mpt_rad_parameters

  subroutine read_mpt_global_parameters(param_dir, mpt_table)
    implicit none
    character(len=*), intent(in) :: param_dir
    character(len=*), intent(in) :: mpt_table
    integer                      :: ierr
    ...
    namelist / mpt_global_parameters / CO2,O2,TIMEAN,FSATMX,Z0SNO,SSI,SNOW_RET_FAC,SNOW_EMIS,&
            SWEMX,TAU0,GRAIN_GROWTH,EXTRA_GROWTH,DIRT_SOOT,&
            BATS_COSZ,BATS_VIS_NEW,BATS_NIR_NEW,BATS_VIS_AGE,BATS_NIR_AGE,BATS_VIS_DIR,BATS_NIR_DIR,&
            RSURF_SNOW,RSURF_EXP
    ...
    if (ierr /= 0) then
       call handle_err(ierr, 'ParametersRead.f90: read_mpt_global_parameters: Cannot find file MPTABLE.TBL')
    endif

    read(15,mpt_global_parameters)
    close(15)
  ...
  END SUBROUTINE read_mpt_global_parameters

  SUBROUTINE read_mpt_crop_parameters(param_dir, mpt_table)
    implicit none
    character(len=*), intent(in) :: param_dir
    character(len=*), intent(in) :: mpt_table
    integer                      :: ierr
    ...
    namelist / mpt_crop_parameters /DEFAULT_CROP,   PLTDAY,     HSDAY,  PLANTPOP,      IRRI,  GDDTBASE,   GDDTCUT,     GDDS1,  GDDS2,  GDDS3,     GDDS4,     GDDS5, & !
    ...
    if (ierr /= 0) then
       call handle_err(ierr, 'ParametersRead.f90: read_mpt_crop_parameters: Cannot find file MPTABLE.TBL')
    endif

    read(15,mpt_crop_parameters)
    close(15)
  ...
  END SUBROUTINE read_mpt_crop_parameters

  SUBROUTINE read_mpt_irrigation_parameters(param_dir, mpt_table)
    implicit none
    character(len=*), intent(in) :: param_dir
    character(len=*), intent(in) :: mpt_table
    integer                      :: ierr
    ...
    namelist / mpt_irrigation_parameters / IRR_FRAC, IRR_HAR, IRR_LAI, IRR_MAD, FILOSS, &
    ...
    if (ierr /= 0) then
       call handle_err(ierr, 'ParametersRead.f90: read_mpt_irrigation_parameters: Cannot find file MPTABLE.TBL')
    endif

    read(15,mpt_irrigation_parameters)
    close(15)
  ...
  END SUBROUTINE read_mpt_irrigation_parameters

  SUBROUTINE read_tiledrain_parameters(param_dir, mpt_table)
    implicit none
    character(len=*), intent(in)    :: param_dir
    character(len=*), intent(in)    :: mpt_table
    integer                         :: ierr
    ...
    namelist / mpt_tiledrain_parameters /DRAIN_LAYER_OPT,TDSMC_FAC,TD_DEPTH,TD_DC,&
    ...
    if (ierr /= 0) then
       call handle_err(ierr, 'ParametersRead.f90: read_mpt_tiledrain_parameters: Cannot find file MPTABLE.TBL')
    endif
    read(15,mpt_tiledrain_parameters)
    close(15)
  ...
  SUBROUTINE read_mpt_optional_parameters(param_dir, mpt_table)
    implicit none
    character(len=*), intent(in) :: param_dir
    character(len=*), intent(in) :: mpt_table
    integer                      :: ierr
    ...
    namelist / mpt_optional_parameters /                                      &
    ...
    if (ierr /= 0) then
       call handle_err(ierr, 'ParametersRead.f90: read_mpt_optional_parameters: Cannot find file MPTABLE.TBL')
    endif

    read(15,mpt_optional_parameters)
    close(15)

  END SUBROUTINE read_mpt_optional_parameters
