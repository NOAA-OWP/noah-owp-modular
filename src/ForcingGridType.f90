...
contains   
  ...
  subroutine ReadForcings(this,datetime_unix,datetime_str)
    ...
    class(forcinggrid_type), intent(inout) :: this
    real*8,intent(in)                      :: datetime_unix                                     ! unix datetime (s since 1970-01-01 00:00:00) ?UTC? 
    ...
    case('YEARLY')
      ...
      this%datetime_file_min = date_to_unix (datetime_long_str)
      ...
      this%datetime_file_max = date_to_unix (datetime_long_str)
    case('MONTHLY')
    ...
    if (.not. lexist) then; write(*,*) 'ERROR Could not find forcings file ''',trim(this%forcings_file_name),''' for datetime ''',trim(datetime_str),'''';stop ":  ERROR EXIT";endif

    !---------------------------------------------------------------------
    ! Open file
    !---------------------------------------------------------------------
    status = nf90_open(path = trim(this%forcings_file_name), mode = nf90_nowrite, ncid = ncid)
    if (status /= nf90_noerr) then; write(*,*) 'ERROR Could not open ''',trim(this%forcings_file_name),''' for datetime ''',trim(datetime_str),''''; stop ":  ERROR EXIT"; endif
    ...
    if(any(time_dif.le.epsilon(datetime_unix))) then
      ...
      write(*,*) 'ERROR Cound not find ''',trim(datetime_str),''' in forcing file ''',trim(this%forcings_file_name),''' -- unix time =',datetime_unix; stop ":  ERROR EXIT"
    end if
    ...
    if(next_datetime_unix.le.this%datetime_file_max) then
      time_dif = abs(this%read_time-next_datetime_unix)
      iread_next = 0
      if(any(time_dif.le.epsilon(next_datetime_unix))) then
        iread_next = minloc(time_dif,1)
      else
        write(*,*) 'ERROR Cound not find second time step in forcing file ''',trim(this%forcings_file_name),''' -- unix time =',next_datetime_unix; stop ":  ERROR EXIT"
      end if
  ...
  subroutine SetForcings(this,datetime_unix,datetime_str)
    ...
    if(datetime_unix.gt.this%datetime_file_max) then
    ...
    ! sanity check
    if(abs(this%read_time(this%iread)-datetime_unix).gt.epsilon(datetime_unix)) then
      write(*,*) 'ERROR Unable to find datetime ''',trim(datetime_str),''' in forcing file ''',trim(this%forcings_file_name),''' - unix time = ',datetime_unix; stop ":  ERROR EXIT"
    end if
