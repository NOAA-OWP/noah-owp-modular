...
contains
  ...
  SUBROUTINE VegeFluxMain (domain, levels, options, parameters, forcing, energy, water)
    ...
    IF((HCAN-energy%ZPD) <= 0.) THEN
      ...
      write(*,*) "CRITICAL PROBLEM IN MODULE VegeFluxMain"
    END IF
  ...
  SUBROUTINE CALHUM (TV, SFCPRS, Q2SAT, DQSDT2)
    ...
    REAL, INTENT(OUT)      :: DQSDT2    ! derivative of Q2SAT with Temperature (not needed? note, is not used anywhere in NOAH-MP)
