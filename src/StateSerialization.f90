module StateSerialization
  ...
  ! Maps model state to and from a MessagePack payload.
  !
  ! Each component is encoded as an array whose positions are the contract --
  ...
  ! Identifies a payload as ours before any of it is applied to live state
...
SUBROUTINE header_serialization (save_datetime, mp_arr)
    real(kind=real64), intent(in) :: save_datetime
...
SUBROUTINE header_deserialization (mp_arr, save_datetime, status)

    ! Reject a payload that is not ours, or is a layout this build cannot read,
    ! before any of it reaches live state
    ...
    call get_real(mp_arr%values(3)%obj, save_datetime, status)

END SUBROUTINE header_deserialization
