module ParametersTypeTransfer
  ...
  subroutine ParametersVarInTransfer(parameters, parametersgrid, ix, iy)
    ...
    parameters%TFRZ = parametersgrid%TFRZ(ix,iy)
    parameters%HSUB = parametersgrid%HSUB(ix,iy)
    parameters%HVAP = parametersgrid%HVAP(ix,iy)
    parameters%HFUS = parametersgrid%HFUS(ix,iy)
    parameters%CWAT = parametersgrid%CWAT(ix,iy)
    parameters%CICE = parametersgrid%CICE(ix,iy)
    parameters%CPAIR = parametersgrid%CPAIR(ix,iy)
    parameters%TKWAT = parametersgrid%TKWAT(ix,iy)
    parameters%TKICE = parametersgrid%TKICE(ix,iy)
    parameters%TKAIR = parametersgrid%TKAIR(ix,iy)
    parameters%RAIR = parametersgrid%RAIR(ix,iy)
    parameters%RW = parametersgrid%RW(ix,iy)
    parameters%DENH2O = parametersgrid%DENH2O(ix,iy)
    parameters%DENICE = parametersgrid%DENICE(ix,iy)
    parameters%THKW = parametersgrid%THKW(ix,iy)
    parameters%THKO = parametersgrid%THKO(ix,iy)
    parameters%THKQTZ = parametersgrid%THKQTZ(ix,iy)
    ...
    parameters%SWEMX = parametersgrid%SWEMX(ix,iy)
    parameters%TAU0 = parametersgrid%TAU0(ix,iy)
    parameters%GRAIN_GROWTH = parametersgrid%GRAIN_GROWTH(ix,iy)
    parameters%EXTRA_GROWTH = parametersgrid%EXTRA_GROWTH(ix,iy)
    parameters%DIRT_SOOT = parametersgrid%DIRT_SOOT(ix,iy)
    parameters%BATS_COSZ = parametersgrid%BATS_COSZ(ix,iy)
    parameters%BATS_VIS_NEW = parametersgrid%BATS_VIS_NEW(ix,iy)
    parameters%BATS_NIR_NEW = parametersgrid%BATS_NIR_NEW(ix,iy)
    parameters%BATS_VIS_AGE = parametersgrid%BATS_VIS_AGE(ix,iy)
    parameters%BATS_NIR_AGE = parametersgrid%BATS_NIR_AGE(ix,iy)
    parameters%BATS_VIS_DIR = parametersgrid%BATS_VIS_DIR(ix,iy)
    parameters%BATS_NIR_DIR = parametersgrid%BATS_NIR_DIR(ix,iy)
    ...
    parameters%BETADS = parametersgrid%BETADS(ix,iy)
    parameters%BETAIS = parametersgrid%BETAIS(ix,iy)
    ...
    parameters%O2 = parametersgrid%O2(ix,iy)
    parameters%CO2 = parametersgrid%CO2(ix,iy)
    parameters%PSIWLT = parametersgrid%PSIWLT(ix,iy)
    parameters%TBOT = parametersgrid%TBOT(ix,iy)
    parameters%GRAV = parametersgrid%GRAV(ix,iy)
  ...
  subroutine ParametersVarOutTransfer(parameters, parametersgrid, ix, iy)
    ...
    parametersgrid%TFRZ(ix,iy) = parameters%TFRZ
    parametersgrid%HSUB(ix,iy) = parameters%HSUB
    parametersgrid%HVAP(ix,iy) = parameters%HVAP
    parametersgrid%HFUS(ix,iy) = parameters%HFUS
    parametersgrid%CWAT(ix,iy) = parameters%CWAT
    parametersgrid%CICE(ix,iy) = parameters%CICE
    parametersgrid%CPAIR(ix,iy) = parameters%CPAIR
    parametersgrid%TKWAT(ix,iy) = parameters%TKWAT
    parametersgrid%TKICE(ix,iy) = parameters%TKICE
    parametersgrid%TKAIR(ix,iy) = parameters%TKAIR
    parametersgrid%RAIR(ix,iy) = parameters%RAIR
    parametersgrid%RW(ix,iy) = parameters%RW
    parametersgrid%DENH2O(ix,iy) = parameters%DENH2O
    parametersgrid%DENICE(ix,iy) = parameters%DENICE
    parametersgrid%THKW(ix,iy) = parameters%THKW
    parametersgrid%THKO(ix,iy) = parameters%THKO
    parametersgrid%THKQTZ(ix,iy) = parameters%THKQTZ
    ...
    parametersgrid%SWEMX(ix,iy) = parameters%SWEMX
    parametersgrid%TAU0(ix,iy) = parameters%TAU0
    parametersgrid%GRAIN_GROWTH(ix,iy) = parameters%GRAIN_GROWTH
    parametersgrid%EXTRA_GROWTH(ix,iy) = parameters%EXTRA_GROWTH
    parametersgrid%DIRT_SOOT(ix,iy) = parameters%DIRT_SOOT
    parametersgrid%BATS_COSZ(ix,iy) = parameters%BATS_COSZ
    parametersgrid%BATS_VIS_NEW(ix,iy) = parameters%BATS_VIS_NEW
    parametersgrid%BATS_NIR_NEW(ix,iy) = parameters%BATS_NIR_NEW
    parametersgrid%BATS_VIS_AGE(ix,iy) = parameters%BATS_VIS_AGE
    parametersgrid%BATS_NIR_AGE(ix,iy) = parameters%BATS_NIR_AGE
    parametersgrid%BATS_VIS_DIR(ix,iy) = parameters%BATS_VIS_DIR
    parametersgrid%BATS_NIR_DIR(ix,iy) = parameters%BATS_NIR_DIR
    ...
    parametersgrid%BETADS(ix,iy) = parameters%BETADS
    parametersgrid%BETAIS(ix,iy) = parameters%BETAIS
    ...
    parametersgrid%O2(ix,iy) = parameters%O2
    parametersgrid%CO2(ix,iy) = parameters%CO2
    parametersgrid%PSIWLT(ix,iy) = parameters%PSIWLT
    parametersgrid%GRAV(ix,iy) = parameters%GRAV
