...
contains   
  ...
  subroutine InitAllocate(this, noahowpgrid)
    ...
    allocate(this%IMELT  (-noahowpgrid%nsnow+1:noahowpgrid%nsoil))
    allocate(this%STC    (-noahowpgrid%nsnow+1:noahowpgrid%nsoil))
    allocate(this%DF     (-noahowpgrid%nsnow+1:noahowpgrid%nsoil))
    allocate(this%HCPCT  (-noahowpgrid%nsnow+1:noahowpgrid%nsoil))
    allocate(this%FACT   (-noahowpgrid%nsnow+1:noahowpgrid%nsoil))
    
    allocate(this%ALBD   (2)) 
    allocate(this%ALBI   (2))
    allocate(this%ALBGRD (2))
    allocate(this%ALBGRI (2)) 
    allocate(this%ALBSND (2)) 
    allocate(this%ALBSNI (2)) 
    
    allocate(this%FABD   (2)) 
    allocate(this%FABI   (2)) 
    allocate(this%FTDD   (2)) 
    allocate(this%FTDI   (2)) 
    allocate(this%FTID   (2)) 
    allocate(this%FTII   (2)) 
    allocate(this%FREVD  (2)) 
    allocate(this%FREVI  (2))  
    allocate(this%FREGD  (2)) 
    allocate(this%FREGI  (2)) 
    allocate(this%RHO    (2)) 
    allocate(this%TAU    (2)) 
