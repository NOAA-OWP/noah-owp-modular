...
contains
  ...
  subroutine paramRead(this, namelist)
    ...
    call read_mp_veg_parameters(namelist%parameter_dir, dataset_identifier)
    call read_mp_soil_parameters(namelist%parameter_dir)
    call read_mp_rad_parameters(namelist%parameter_dir)
    call read_mp_global_parameters(namelist%parameter_dir)
...
!---------------------------------------------------------------------
    ...
    this%Z0           = 0.002  ! bare soil roughness length (m).  NOTE: This is hard-coded in f90 in hrldas version noahmp
